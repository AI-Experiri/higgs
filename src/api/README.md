...
- Hub/fleet: `nodes()`, `pair()`, `hub_enable()` / `hub_disable()`, `node_load` /
  `node_unload` / `node_retire` / `node_label` / `node_scan`, plus the installers
  `set_fleet`/`fleet`, `set_hub`/`hub`/`clear_hub`.
- Keys/security: `mint_key(label, scopes?)` / `revoke_key(label)` (trusted, bearer-free
  but every structural invariant intact), `set_api_keys`/`api_keys`/`mutate_api_keys`,
  `register_internal_token` (the embedder's hidden in-memory token), `set_lan_exposed`,
  `extra_cors_origins`.
