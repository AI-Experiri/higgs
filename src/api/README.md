...
| File | Responsibility |
|------|----------------|
| `api.rs` | The `Higgs` struct + its core `impl` (construct, `load`/`unload`/`status`/`chat_stream`/`scan`/`probe_support`/`events`/`logs`/`sysinfo`/`stop`/`start`, the served-id helpers `local_served`/`local_served_ids`/`local_loaded_info`), plus submodule declarations and re-exports. Delegates to the `local: Arc<NodeRuntime>` field. |
