...
| `control.rs` | The `/api/higgs/*` handlers: models list (+ host-side `tool_calls` support verdict per model), single model by id, load/unload, status, system, logs (snapshot + SSE stream), log/runtime settings GET/PUT, version, worker stop, and the hub/fleet routes (pair, nodes, node load/unload/models/retire). |
