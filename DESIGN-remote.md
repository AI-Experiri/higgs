...
{ "role": "node",                         // "node"|"hub" — self-declared, cross-checked vs allowlist
  ...
  "protocol_versions": [1],               // every wire-protocol major this peer can SPEAK
...
{ "role": "hub",
  ...
  "agreed_version": 1,                     // the single major both sides pin for this session
...
|---|---|---|---|---|
| `M_LOAD` | `higgs/node/load` | `LoadParams` (`id`,`ctx_len?`,`gpu_layers?`,`threads?`,`idle_ttl_minutes?`) | `{ "worker_id", "loaded": LoadedInfo }` | **fit-check VRAM** (fix below) → spawn a NEW `Supervisor` → assign a NEW `WorkerId` → insert into registry → load model |
...
| Standing | allowlist membership (QUIC/TLS proves peer key) | `Authorization: Bearer sk-higgs-…` |
| At rest | `pairings.json` (`Vec<EndpointId>` + label) | `api_keys.json` (SHA-256 hash, prefix, scopes) — never plaintext |
| Checked | transport-level, **post-HELLO** via `remote_id()` (§3.2) | app-level middleware on `/v1` + `/api/higgs/*` |
| CLI noun | `link …` | `keys …` |
...
```
# hub side (surface A — fleet of machines)
higgs link enable            # start the iroh hub listener (accept node dials)
higgs link disable           # stop accepting; existing sessions drain
...
higgs link ls                # paired nodes: id, label, sw version, online, loaded workers
...
higgs keys create --name X --scope chat   # mint Bearer, print sk-higgs-… ONCE
higgs keys ls
higgs keys revoke <id>
```
