...
//!
//! Spawns the real `higgs-server` binary on a localhost port, waits for the
...
/// A running `higgs-server` child process. Killed (with its worker) on drop.
...
pub async fn spawn(port: u16) -> ServerGuard {
    ...
    panic!("higgs-server never became ready on {base}");
}
