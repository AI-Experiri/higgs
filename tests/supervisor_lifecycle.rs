...
//! driven through the in-process `Higgs` crate API (library-first: control =
//! facade methods, chat = the real `/v1` HTTP surface via `serve_v1_local`).
//!
...
//! lifecycle paths against the tiny on-disk model — the ones that do NOT need a
//! wedged worker to time out (`CONTROL_RPC_TIMEOUT`/`CHAT_RPC_TIMEOUT` are
//! 120s/600s) or fault injection:
//!
...
use common::{higgs_local, serve_v1_local};
use serde_json::{json, Value};

/// PIDs of the `--higgs-worker` children of THIS test process — one OS child per
/// loaded model (workers spawn from the real `higgs` binary via the `worker_exe`
/// seam, so they are direct children of the in-process test runtime). Confirms
/// each worker is a distinct real process, not just a distinct `worker_id`.
...
/// Sorted resident model ids from a status snapshot's `loaded_all`.
fn loaded_ids(s: &higgs::HiggsStatus) -> Vec<String> {
    let mut v: Vec<String> = s.loaded_all.iter().map(|e| e.id.clone()).collect();
    v.sort();
    v
}

/// Non-streaming chat against `model` over the real `/v1` HTTP surface; asserts a
/// string content came back (drives that worker's reader/writer + M_CHAT). Kept
/// non-streaming so no SSE stream is ever left open across teardown.
async fn chat_ok(c: &reqwest::Client, base: &str, model: &str) {
    let resp: Value = c
    ...
    assert!(
        ...
        "chat against worker {model} returns content: {resp}"
    );
}
...
async fn multi_worker_coexist_unload_one_reload_and_chat() {
    ...
    // Real `/v1` surface for the chat round-trips (the control paths use the facade).
    ...
    for id in ids {
        higgs
            .load(id, None)
            .await
            .unwrap_or_else(|e| panic!("load {id} ok: {e:?}"));
    }

    // ── Each model is a coexisting worker: distinct worker_id, own child PID ────
    let s = higgs.status().await.unwrap();
    assert_eq!(
        loaded_ids(&s),
        ["sup/a", "sup/b", "sup/c"],
        "all three models resident as separate workers: {s:?}"
    );
    assert!(s.worker_alive, "primary worker answers M_STATUS: {s:?}");
    let worker_ids: std::collections::HashSet<u32> =
        s.loaded_all.iter().map(|e| e.worker_id).collect();
    assert_eq!(
        worker_ids.len(),
        3,
    ...
    // `--higgs-worker` per `do_spawn`, so the test process has 3 distinct worker kids.
    ...
    for id in ids {
        chat_ok(&c, &base, id).await;
    }

    // ── Per-id unload: stop ONLY sup/b's worker; sup/a and sup/c survive ───────
    // `unload_one` → `Supervisor::stop()` of that ONE worker — a graceful per-worker
    // stop, NOT the drain-all path. Local dedup ⇒ served id == raw id.
    higgs
        .unload_one("sup/b")
        .await
        .expect("per-id unload of sup/b");
    ...
    // the sibling's stop().
    chat_ok(&c, &base, "sup/a").await;

    // ── Explicit RELOAD of the unloaded id → a fresh worker (control-load path) ─
    higgs.load("sup/b", None).await.expect("reload of sup/b");
    ...
    higgs.load("sup/a", None).await.expect("idempotent reload");
    let s = higgs.status().await.unwrap();
    assert_eq!(
        loaded_ids(&s),
        ["sup/a", "sup/b", "sup/c"],
    ...
    higgs
        .worker_stop()
        .await
        .expect("worker/stop drains all workers");
    let s = higgs.status().await.unwrap();
    assert!(
        !s.worker_alive,
    ...
    assert!(
        s.loaded_all.is_empty(),
        "loaded_all is empty after the bulk stop: {s:?}"
    );

    // Tear down the /v1 server BEFORE the facade (never leave a stream open).
