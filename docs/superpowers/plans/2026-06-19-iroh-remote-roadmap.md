...
| **P3** | #14 | _(in progress)_ | ✅ **node-side chat relay** (`data::relay_chat`) + full `M_NODE_SYSINFO` (cpu/mem/gpu) + hermetic `HIGGS_IROH_LOCAL` + **real-process e2e** (`tests/remote_node_e2e.rs`: spawn `higgs --node` → load real GGUF → sysinfo → chat → status; gate 90.64%). ⏳ Remaining: `remote_factory`+`spawn_remote`; `WorkerProc` trait replacing `proc:Option<Child>` (7 sites); hub per-node iroh transport (own pending/correlation keyed by `NodeId`); `/v1`→remote wiring; wedged-worker reap; HG027. **Exit:** `/v1` chat to remote-resident worker streams back; wedged worker escalates `M_KILL`→redial→retire (HG027). | none | **partial** |
