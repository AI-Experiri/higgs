...
| **P2** | #13 | _(plan after P1)_ | `src/node.rs`: bind+accept loop+post-HELLO gate; `NodeRuntime{ HashMap<WorkerId,Arc<Supervisor>> }` + `WorkerId(u32)`; node-side CONTROL dispatch (registry ops); outbound hello/inventory requester; DATA bridge per worker→`serve_state` (`SyncIoBridge` per stream in `spawn_blocking`, under the node's **multi-thread** runtime — this is also where the **worker tokio port deferred from P0** lands, §5.3); `--node`/`node connect`; `M_LOAD` spawns 2nd+ Supervisor w/ VRAM fit-check; `M_KILL`/`M_UNLOAD` free `WorkerId`. **Exit:** node hosts 2 concurrent workers, `M_SYSINFO`+`M_STATUS` over iroh. | `toml` | pending |
